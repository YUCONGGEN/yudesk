...
(async()=>{
  ...
  try{
    ...
    for(const page of [left,right]){
      ...
      assert.ok(await page.evaluate(()=>conference.audioMeters.size>0),'remote meeting audio must feed a real audio-level analyser');
      assert.deepEqual(await page.evaluate(()=>{const id=[...conference.states.keys()].find(value=>value!==conference.selfID);setConferenceVoiceLevel(id,82);const tile=conference.tiles.get(id),bars=[...tile.querySelectorAll('.conference-voice i')];return [tile.classList.contains('speaking'),tile.querySelector('.conference-voice').classList.contains('active'),Math.max(...bars.map(bar=>parseInt(bar.style.height,10)))>=11];}),[true,true,true]);
    }
    ...
    const conferenceFullscreenCommands=[];
    const conferenceFullscreenMatch=u=>u.pathname==='/api/ui/fullscreen';
    await right.route(conferenceFullscreenMatch,route=>{conferenceFullscreenCommands.push(route.request().postDataJSON());return route.fulfill({status:204,body:''});});
    await right.locator('#conferenceFullscreen').click();
    await right.waitForFunction(()=>document.body.classList.contains('conference-fullscreen')&&conference.fullscreenMode==='native');
    assert.match(await right.locator('#conferenceFullscreen').textContent(),/退出全屏/);
    const escapeRestore=right.waitForResponse(response=>new URL(response.url()).pathname==='/api/ui/fullscreen'&&response.request().postDataJSON().enabled===false);
    await right.keyboard.press('Escape');
    await escapeRestore;
    await right.waitForFunction(()=>!document.body.classList.contains('conference-fullscreen')&&!conference.fullscreenMode);
    assert.deepEqual(conferenceFullscreenCommands,[{enabled:true},{enabled:false}],'Escape must leave native conference fullscreen');
    ...
    await right.locator('#conferenceMemberSearch').fill('参会');
    assert.match(await right.locator('#conferenceMemberCount').textContent(),/1 \/ 2 人/);
    await right.locator('#conferenceMemberSearch').fill('不存在');
    assert.equal(await right.locator('#conferenceMemberEmpty').isVisible(),true,'member search must expose an empty state');
    await right.locator('#conferenceMemberSearch').fill('');
    const guestMember=right.locator('.conference-member').filter({hasText:'参会者'});
    await guestMember.locator('button').click();
    await right.locator('.conference-member-menu [data-member-action="info"]').click();
    assert.match(await right.locator('.conference-member-info').textContent(),/参会者[\s\S]*参会成员[\s\S]*加入时间/);
    await right.locator('.conference-member-info button').click();
    await guestMember.locator('button').click();
    await right.locator('.conference-member-menu [data-member-action="transfer"]').click();
    ...
    await right.locator('#conferenceSharing').waitFor({state:'visible'});
    assert.match(await right.locator('#conferenceSharing').textContent(),/参会者（我）|参会者/);
    const sharedTile=right.locator('.conference-tile.shareable');
    await sharedTile.click();
    await right.waitForFunction(()=>document.getElementById('conferenceGrid').classList.contains('conference-grid-focused')&&document.querySelectorAll('.conference-tile.conference-focused').length===1);
    await sharedTile.click();
    await right.waitForFunction(()=>!document.getElementById('conferenceGrid').classList.contains('conference-grid-focused'));
    ...
    const fullscreenCommandsBeforeEnd=conferenceFullscreenCommands.length;
    await right.locator('#conferenceFullscreen').click();
    await right.waitForFunction(()=>conference.fullscreenMode==='native');
    const endingRestore=right.waitForResponse(response=>new URL(response.url()).pathname==='/api/ui/fullscreen'&&response.request().postDataJSON().enabled===false);
    ...
    await right.waitForFunction(()=>!document.body.classList.contains('conference-fullscreen'));
    assert.equal(conferenceFullscreenCommands.length,fullscreenCommandsBeforeEnd+2,'ending must add one fullscreen enter/restore pair');
    assert.deepEqual(conferenceFullscreenCommands.slice(-2),[{enabled:true},{enabled:false}],'ending the meeting must restore the original native window size');
    assert.equal(conferenceFullscreenCommands.filter(value=>value.enabled).length,conferenceFullscreenCommands.filter(value=>!value.enabled).length,'every meeting fullscreen entry must be restored');
    await right.unroute(conferenceFullscreenMatch);
    ...
    const fullscreenMatch=u=>u.pathname==='/api/ui/fullscreen';
    await left.route(fullscreenMatch,route=>{fullscreenCommands.push(route.request().postDataJSON());return route.fulfill({status:204,body:''});});
    await left.locator('#fullscreen').click();
    await left.waitForFunction(()=>document.body.classList.contains('remote-fullscreen'));
    assert.equal(await left.locator('#exitFullscreen').isVisible(),true);
    assert.deepEqual(await left.locator('#desktop').evaluate(element=>{const box=element.getBoundingClientRect();return [Math.round(box.width),Math.round(box.height),Math.round(box.x),Math.round(box.y)];}),[860,562,0,0]);
    await left.locator('#exitFullscreen').click();
    await left.waitForFunction(()=>!document.body.classList.contains('remote-fullscreen'));
    assert.deepEqual(fullscreenCommands,[{enabled:true},{enabled:false}]);
    await left.unroute(fullscreenMatch);
