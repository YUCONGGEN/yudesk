...
(async()=>{
  ...
  try{
    ...
    const checkWindowActions=async(page,hideSelector)=>{
      const selectors=[hideSelector,'[data-window-action="minimize"]','#closeToTray'];
    ...
    assert.equal(await right.locator('#conferenceFullscreen').isVisible(),true,'exit fullscreen control must remain visible in native fullscreen');
    ...
    const hostShareFullscreenCommands=[];
    await left.route(conferenceFullscreenMatch,route=>{hostShareFullscreenCommands.push(route.request().postDataJSON());return route.fulfill({status:204,body:''});});
    await left.locator('#conferenceFullscreen').click();
    await left.waitForFunction(()=>conference.fullscreenMode==='native');
    await left.locator('#conferenceShare').click();
    await left.waitForFunction(()=>conference.screen&&!conference.shareBusy&&conference.fullscreenMode==='native');
    assert.equal(await left.locator('#conferenceShare').isEnabled(),true,'screen share control must not remain grey after selection');
    assert.deepEqual(hostShareFullscreenCommands,[{enabled:true},{enabled:false},{enabled:true}],'sharing from fullscreen must reveal the system picker and restore fullscreen afterwards');
    ...
    await left.locator('#conferenceFullscreen').click();
    await left.waitForFunction(()=>!conference.fullscreenMode);
    assert.deepEqual(hostShareFullscreenCommands,[{enabled:true},{enabled:false},{enabled:true},{enabled:false}],'host fullscreen transitions must stay balanced');
    await left.unroute(conferenceFullscreenMatch);
