...
public class PointerControllerTest {
    ...
    @Test public void trackpadMoveDoesNotHoldButton() throws Exception {Wire w=new Wire();PointerController p=pointer(w);p.down(100,100,0);p.move(200,120,16);p.up(220,130,20);assertEquals(Arrays.asList("move","move"),w.types());assertEquals(1240,p.x());assertEquals(537,p.y());}
    @Test public void dragIsOneGestureAndFinalPositionPrecedesRelease() throws Exception {Wire w=new Wire();PointerController p=pointer(w);p.drag(true);p.down(100,100,0);p.move(200,100,1);p.up(220,100,2);assertEquals(Arrays.asList("move","down","move","move","up"),w.types());assertEquals(1200,w.events.get(2).x);assertEquals(1240,w.events.get(4).x);assertFalse(p.dragging());assertFalse(p.active());}
    @Test public void directClickPositionsMouseBeforeDown() throws Exception {Wire w=new Wire();PointerController p=pointer(w);p.mode(false);p.down(250,300,0);p.up(250,300,1);assertEquals(Arrays.asList("move","down","move","up"),w.types());assertEquals(500,w.events.get(0).x);assertEquals(375,w.events.get(0).y);}
    @Test public void directFillMapsEveryVisiblePoint() throws Exception {Wire w=new Wire();PointerController p=pointer(w);p.mode(false);p.down(100,10,0);p.up(100,10,50);assertEquals(Arrays.asList("move","down","move","up"),w.types());assertEquals(200,w.events.get(0).x);}
    ...
    @Test public void directDrawingRetainsEveryFastZigzagSample() throws Exception {
        Wire w=new Wire();PointerController p=new PointerController(w,8);p.geometry(new ScreenMapping(256,256,256,256));p.mode(false);p.down(20,20,0);
        for(int i=1;i<=8;i++)p.move(i%2==0?20:180,20+i*4,i);p.up(180,60,9);
        assertEquals(12,w.events.size());
        for(int i=1;i<=8;i++){PointerController.Event sample=w.events.get(i+1);assertEquals("move",sample.type);assertEquals(i%2==0?20:180,sample.x);assertEquals(20+i*4,sample.y);}
        assertEquals("up",w.events.get(11).type);assertFalse(p.holdingButton());
    }
    @Test public void explicitRelativeDragDoesNotLoseSubSlopSamples() throws Exception {
        Wire w=new Wire();PointerController p=new PointerController(w,8);p.geometry(new ScreenMapping(256,256,256,256));p.drag(true);p.down(100,100,0);
        p.move(101,100,1);p.move(101,101,2);p.up(100,101,3);
        assertEquals(Arrays.asList("move","down","move","move","move","up"),w.types());
        assertEquals(129,w.events.get(2).x);assertEquals(128,w.events.get(2).y);assertEquals(129,w.events.get(3).y);
    }
}
