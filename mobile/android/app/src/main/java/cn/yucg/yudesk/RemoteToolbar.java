...
final class RemoteToolbar {
    ...
    void update(JSONObject session){boolean control=session.optBoolean("control"),ready=session.optBoolean("ready")&&!session.optBoolean("closed");long rtt=session.optLong("rttMs");String message=session.optString("message");String label=message.isEmpty()||message.equals("已连接")?(control?"远程控制":"仅观看"):message;status.setText(context.getString(R.string.remote_status,label,rtt>0?rtt+" ms":"测量中"));status.setContentDescription(message+"；网络往返延迟 "+rtt+" 毫秒，不是画面延迟");for(Button b:new Button[]{mode,left,right,drag,keyboard}){b.setEnabled(control&&ready);b.setAlpha(control&&ready?1:.4f);}syncInput();}
