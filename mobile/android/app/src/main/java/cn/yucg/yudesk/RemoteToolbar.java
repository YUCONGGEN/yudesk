...
final class RemoteToolbar {
    ...
    private final Button rotate,mode,left,right,drag,keyboard,finish;
    ...
    RemoteToolbar(Context context,RemoteView remote,Runnable rotation,Runnable tools,Runnable end,Runnable text){
        ...
        finish=button("结束",end);finish.setTextColor(0xffffaab0);top.addView(finish,new LinearLayout.LayoutParams(dp(52),dp(44)));
    ...
    void update(JSONObject session){boolean meeting=session.optBoolean("meeting"),control=session.optBoolean("control"),ready=session.optBoolean("ready")&&!session.optBoolean("closed");long rtt=session.optLong("rttMs");String message=session.optString("message");String label=meeting?"会议观看":message.isEmpty()||message.equals("已连接")?(control?"远程控制":"仅观看"):message;String transport=session.optString("transport"),route="p2p".equals(transport)?"P2P 直连":"udp-relay".equals(transport)?"UDP 中转":"TCP 中转";status.setText(context.getString(R.string.remote_status,label,route+" · "+(rtt>0?rtt+" ms":"测量中")));status.setContentDescription(message+"；"+route+"；网络往返延迟 "+rtt+" 毫秒，不是画面延迟；"+session.optString("transportReason"));finish.setText(meeting?"离会":"结束");for(Button b:new Button[]{mode,left,right,drag,keyboard}){b.setEnabled(control&&ready);b.setAlpha(control&&ready?1:.4f);}syncInput();}
