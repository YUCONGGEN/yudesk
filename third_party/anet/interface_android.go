...
func interfaceTable(ifindex int) ([]net.Interface, error) {
	...
	msgs, err := ParseNetlinkMessage(tab)
	if err != nil {
	...
	for _, m := range msgs {
		...
		case syscall.RTM_NEWADDR:
			if len(m.Data) < syscall.SizeofIfAddrmsg {
				return nil, os.NewSyscallError("ifaddrmsg", syscall.EINVAL)
			}
...
func addrTable(ifi *net.Interface, msgs []NetlinkMessage) ([]net.Addr, error) {
	var ifat []net.Addr
loop:
	for _, m := range msgs {
		switch m.Header.Type {
...
func newAddr(ifam *syscall.IfAddrmsg, attrs []NetlinkRouteAttr) net.Addr {
	if ifam == nil {
		return nil
	}
	...
	for _, a := range attrs {
		...
		case syscall.AF_INET:
			if len(a.Value) != net.IPv4len || ifam.Prefixlen > 32 {
				return nil
			}
		...
		case syscall.AF_INET6:
			if len(a.Value) != net.IPv6len || ifam.Prefixlen > 128 || ifam.Index == 0 {
				return nil
			}
			ifa := &net.IPNet{IP: make(net.IP, net.IPv6len), Mask: net.CIDRMask(int(ifam.Prefixlen), 8*net.IPv6len)}
			copy(ifa.IP, a.Value[:])
			return addressWithNumericZone(ifa, int(ifam.Index))
		}
	}
	return nil
}
